#!/usr/bin/env python3
"""
AEON Hierarchical Schema Fix v2: Complete Migration to v3.1 (Constraint-Safe)
...
- Handles uniqueness constraints safely using CALL {...} IN TRANSACTIONS
- Processes nodes in batches to avoid constraint violations
- Adds detailed progress reporting
- Implements transaction-safe super label addition

File: FIX_HIERARCHICAL_SCHEMA_V2.py
Created: 2025-12-12
Version: 2.0.0
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/home/jim/2_OXOT_Projects_Dev/7_2025_DEC_11_Actual_System_Deployed/logs/schema_fix_v2.log'),
...
class HierarchicalSchemaFixV2:
    """
    Complete migration to Neo4j v3.1 hierarchical schema (constraint-safe version).

    Phases:
    1. Analysis - Count affected nodes
    2. Super Labels - Add to existing nodes (constraint-safe)
    ...
    # 17 Super Labels from v3.1 Schema (+ Measurement)
    SUPER_LABELS = {
        ...
        "Measurement": ["Measurement", "ManufacturingMeasurement", "DefenseMeasurement", "HealthcareMeasurement", "ITMeasurement", "NetworkMeasurement", "RadiationMeasurement", "SAREF_Measurement", "ChemicalMeasurement", "GovernmentMeasurement", "SoilMeasurement"]
    }
    ...
    # =========================================================================
    # PHASE 2: ADD SUPER LABELS (CONSTRAINT-SAFE)
    # =========================================================================

    def add_super_labels(self) -> Dict:
        """
        Add super labels to all nodes based on existing label patterns.
        Uses CALL {...} IN TRANSACTIONS for constraint safety.

        Returns:
            Results with counts of labels added
        """
        logger.info("="*80)
        logger.info("PHASE 2: ADDING SUPER LABELS TO EXISTING NODES (CONSTRAINT-SAFE)")
        logger.info("="*80)
        ...
        with self.driver.session() as session:
            for super_label, patterns in self.SUPER_LABELS.items():
                ...
                # Use CALL {...} IN TRANSACTIONS for batch processing
                query = f"""
                CALL {{
                    MATCH (n)
                ...
                try:
                    ...
                    logger.error(f"  {super_label}: Error adding labels - {e}")
                    results[super_label] = 0
    ...
    def add_hierarchical_properties(self) -> Dict:
        ...
        with self.driver.session() as session:
            for super_label, tier1 in self.TIER1_MAPPING.items():
                # Add tier1, tier2, tier properties using batch processing
                query = f"""
                CALL {{
                    MATCH (n:{super_label})
                ...
                try:
                    ...
                    logger.error(f"  {super_label}: Error adding properties - {e}")
                    results[super_label] = 0
        ...
        with self.driver.session() as session:
            for super_label, discriminator_property in self.DISCRIMINATORS.items():
                ...
                try:
                    ...
                    logger.error(f"  {super_label}: Error adding discriminators - {e}")
                    results[super_label] = 0
    ...
    def run_complete_migration(self) -> Dict:
        ...
        """
        logger.info("="*80)
        logger.info("AEON HIERARCHICAL SCHEMA FIX V2 - COMPLETE MIGRATION TO v3.1")
        logger.info("="*80)
...
def main():
    """Main execution"""
    print("="*80)
    print("AEON HIERARCHICAL SCHEMA FIX V2 (CONSTRAINT-SAFE)")
    print("Complete Migration to Neo4j v3.1 Schema")
    print("="*80)
    print()
    print("This script will:")
    print("1. Analyze current database state")
    print("2. Add super labels to all 1.2M nodes (constraint-safe)")
    ...
    print("- Uses CALL {...} IN TRANSACTIONS for batch processing")
    print("- Handles uniqueness constraints safely")
    print("- Continues on errors with ON ERROR CONTINUE")
    print()
    ...
    with HierarchicalSchemaFixV2() as fixer:
        ...
        if result["success"]:
            ...
            print("="*80)
            print(f"See logs for details: /home/jim/2_OXOT_Projects_Dev/7_2025_DEC_11_Actual_System_Deployed/logs/schema_fix_v2.log")
        else:
            print()
            print("="*80)
