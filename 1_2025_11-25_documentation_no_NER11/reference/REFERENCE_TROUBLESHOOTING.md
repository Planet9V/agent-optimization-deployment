...
## SECTION 8: TROUBLESHOOTING DECISION TREES

### Decision Tree 1: API Returns 500 Error
...
│  │  ├─ Is Database Running?
│  │  │  │
│  │  │  ├─ YES → Check Firewall/Network
│  │  │  │  │
│  │  │  │  ├─ Can ping database host?
│  │  │  │  │  ├─ YES → Check database port accessibility
│  │  │  │  │  │        `telnet neo4j-host 7687`
│  │  │  │  │  │        If fails: Firewall blocking connection
│  │  │  │  │  │        Solution: Update security group rules
│  │  │  │  │  │
│  │  │  │  │  └─ NO → Network connectivity issue
│  │  │  │  │           Solution: Check VPC routing, DNS resolution
│  │  │  │  │
│  │  │  └─ NO → Database Stopped
│  │  │     │
│  │  │     ├─ Local Development: `docker-compose restart neo4j`
│  │  │     │   Wait 30 seconds, retry request
│  │  │     │
│  │  │     └─ Production: Check K8s pod status
│  │  │         `kubectl get pods -n wave4-production | grep neo4j`
│  │  │         If CrashLoopBackOff: Check logs
│  │  │         `kubectl logs <pod-name> -n wave4-production`
│  │  │
...
│  │  │  │  Solution: Reset password or update config
│  │  │  │
│  │  │  └─ Check Kubernetes secret
│  │  │     `kubectl get secret databases -n wave4-production -o yaml`
│  │  │     Decode base64: `echo <value> | base64 -d`
│  │  │
│  ├─ "Out of memory" OR "Heap space"
│  │  │
│  │  ├─ Check Memory Usage
│  │  │  `docker stats` (local) OR `kubectl top pods` (k8s)
│  │  │
│  │  ├─ If > 90% memory used
│  │  │  │
...
│  │  │  │
│  │  │  └─ Long-term: Increase memory allocation
│  │  │     - Docker: Update docker-compose.yml mem_limit
│  │  │     - K8s: Update deployment resources.limits.memory
│  │  │
│  └─ "Query timeout" OR "Transaction timeout"
│     │
│     ├─ Check Query Complexity
│     │  │
│     │  ├─ Is query too complex?
│     │  │  Review query execution time in logs
│     │  │  If > 5 seconds: Query needs optimization
│     │  │
│     │  ├─ Add LIMIT clause
...
│        - FastAPI: Update request timeout in settings
│        - Neo4j: Update transaction timeout
│          `dbms.transaction.timeout=300s`
```

---

### Decision Tree 2: Query Returns Empty Results

```
Query Returns No Results (Expected Data Present)
│
...
│  │  │
│  │  ├─ Count = 0 → Data Not Loaded
│  │  │  │
│  │  │  ├─ Check Data Import Status
│  │  │  │  Review import logs: `docker-compose logs backend | grep import`
│  │  │  │
│  │  │  ├─ Re-run Data Import
...
│  │  │
│  │  └─ Count > 0 → Data Exists, Query Issue
│  │     │
│  │     ├─ Check Query Filters
│  │     │  │
│  │     │  ├─ Are filters too restrictive?
│  │     │  │  Example: `WHERE country = 'Russia' AND active = false`
│  │     │  │  Test without filters: `MATCH (n:ThreatActor) RETURN n LIMIT 10`
│  │     │  │
│  │     │  ├─ Check Case Sensitivity
│  │     │  │  Neo4j is case-sensitive for property values
│  │     │  │  'Russia' ≠ 'russia'
│  │     │  │  Solution: Use toLower() for comparison
│  │     │  │  `WHERE toLower(n.country) = 'russia'`
│  │     │  │
│  │     │  └─ Check Data Type Mismatch
│  │     │     Example: Filtering year as string instead of int
│  │     │     `WHERE n.year = '2020'` → fails if year stored as int
│  │     │     Solution: `WHERE n.year = 2020`
│  │     │
│  │     └─ Check Relationship Direction
│  │        │
│  │        ├─ Relationship may be reversed
│  │        │  `(a)-[:EXPLOITS]->(b)` vs `(a)<-[:EXPLOITS]-(b)`
│  │        │
│  │        └─ Test with undirected relationship
│  │           `(a)-[:EXPLOITS]-(b)` (no arrow = bidirectional)
│  │           If returns results: Direction was incorrect
│  │
│  └─ Check API Permissions
│     │
│     ├─ User may lack read permission
│     │  `GET /api/v1/users/me` → Check role and scopes
│     │
│     └─ If role = 'viewer' but scope missing 'read:threat_intel'
│        Solution: Contact admin to grant scope
```

---

### Decision Tree 3: Neo4j Out of Memory
...
│  │  │         value: "4G"
│  │  │     ```
│  │  │     Apply: `kubectl apply -f neo4j-deployment.yaml`
│  │  │
│  │  └─ Recommended Heap Size
│  │     - Small dataset (<10GB): 2-4GB heap
│  │     - Medium dataset (10-100GB): 4-8GB heap
│  │     - Large dataset (>100GB): 8-16GB heap
│  │     - Rule: Heap should be 25-50% of container memory
│  │
│  └─ Page Cache Memory
│     │
...
│     │  ```
│     │
│     └─ Recommended Page Cache Size
│        - Should be ~50-75% of container memory
│        - Larger than database store size if possible
│        - Example: 16GB container → 4GB heap + 10GB page cache
│
...
│  │     Example: `MATCH (a), (b) RETURN a, b`
│  │     If 1000 nodes each: Returns 1M results!
│  │     Solution: Add relationship between a and b
...
│
└─ Monitor & Prevent Future Issues
   │
   ├─ Set Up Monitoring
   │  - Prometheus metrics: neo4j_memory_heap_used_bytes
   │  - Alert when > 85% heap used
   │
   └─ Query Optimization
...
### Decision Tree 4: Authentication Failures
...
│  │  │  │  Query database: `SELECT username FROM users WHERE username = 'admin'`
│  │  │  │  If not found: User doesn't exist
│  │  │  │  Solution: Create user or use correct username
│  │  │  │
...
│  │  │  - JWT secret not set
│  │  │  - Password hashing library error
│  │  │
│  │  └─ 200 OK → Credentials Valid, Token Returned
│  │     Proceed to check token usage
│  │
│  └─ Check JWT Secret Configuration
│     │
│     ├─ Verify JWT_SECRET environment variable set
│     │  `docker-compose exec backend env | grep JWT_SECRET`
│     │  If empty: JWT tokens cannot be signed
│     │  Solution: Set JWT_SECRET in .env file
│     │
│     └─ Verify JWT secret same across all backend instances
│        In production, all API pods must use same secret
│        Otherwise tokens signed by pod A won't validate on pod B
│
├─ Check Token Usage
│  │
│  ├─ Token Expired?
...
│  │  │
│  │  └─ If exp > current time → Token Valid
│  │     Issue elsewhere (check below)
│  │
...
└─ Check Backend Configuration
   ...
   ├─ OAuth2 Scheme Configured?
      │
      ├─ Check FastAPI oauth2_scheme
      │  ```python
      │  oauth2_scheme = OAuth2PasswordBearer(
      │    tokenUrl="/api/v1/users/token"
      │  )
      │  ```
      │  TokenUrl must match login endpoint
      │
      └─ Check CORS Settings
         │
...
### Decision Tree 5: Deployment Fails

```
Kubernetes Deployment Fails
│
├─ Check Pod Status
│  `kubectl get pods -n wave4-production`
│  │
│  ├─ ImagePullBackOff
│  │  │
│  │  ├─ Image Doesn't Exist
...
│  │  │  │
│  │  │  ├─ Check Image Name
│  │  │  │  Verify: registry/repository:tag format
│  │  │  │  Example: 534359854279.dkr.ecr.us-east-1.amazonaws.com/wave4/backend:latest
│  │  │  │
│  │  │  └─ Check Image Exists in Registry
│  │  │     ```bash
│  │  │     aws ecr describe-images \
│  │  │       --repository-name wave4/backend \
│  │  │       --image-ids imageTag=latest
│  │  │     ```
│  │  │     If not found: Build and push image first
│  │  │
│  │  └─ Registry Authentication Failed
│  │     │
│  │     ├─ Check Image Pull Secret
│  │     │  `kubectl get secrets -n wave4-production`
│  │     │  Look for: regcred or ecr-credentials
│  │     │
│  │     └─ Create Image Pull Secret (ECR)
│  │        ```bash
│  │        kubectl create secret docker-registry ecr-credentials \
│  │          --docker-server=534359854279.dkr.ecr.us-east-1.amazonaws.com \
│  │          --docker-username=AWS \
...
│  │  │  │  │
│  │  │  │  ├─ "Environment variable not set" → Missing config
│  │  │  │  │  Check deployment env variables
│  │  │  │  │  Verify secrets exist and mounted
│  │  │  │  │
...
│  │  │     `kubectl logs <pod-name> -n wave4-production --previous`
...
│  │        App may need more time to start
│  │        Change: initialDelaySeconds: 10 → 30
│  │
│  ├─ Pending
│  │  │
...
│  │  │       --cluster-name wave4-production \
│  │  │       --nodegroup-name wave4-node-group \
│  │  │       --scaling-config desiredSize=5
│  │  │     ```
│  │  │
│  │  └─ Volume Mount Failed
│  │     Check PersistentVolumeClaim status
│  │     `kubectl get pvc -n wave4-production`
│  │     If Pending: PV not available
│  │
│  └─ Error
│     │
│     └─ Check Event Log
...
└─ Check Deployment Configuration
   ...
   │  `kubectl get namespace wave4-production`
   │  If not found: `kubectl create namespace wave4-production`
   │
   └─ Verify Secrets Exist
      `kubectl get secrets -n wave4-production`
      Required: databases, api-secrets
      If missing: Create secrets (see deployment guide)
```

---

**End of Troubleshooting Guide** (~1,100 lines with decision trees)
