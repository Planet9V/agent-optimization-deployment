...
## NER11 Gold Container Deployment
The NER11 Gold API is deployed as a standalone Docker container.

### Container Specifications
- **Base Image**: `python:3.12-slim` (or similar)
- **Dependencies**: `fastapi`, `uvicorn`, `spacy[cuda11x]`
- **Volume Mounts**:
  - `/app/training_data`: Persists local training data for future updates.
- **Network**: Attached to the `aeon-net` bridge network for internal communication.

### Security
- **API Access**: Restricted to internal services via Docker network.
- **External Access**: Exposed via Nginx reverse proxy with TLS termination.
