...
export class AgentDB {
  ...
  /**
   * Generate fallback embedding when embedding service fails
   * Uses a deterministic hash-based approach to create a consistent vector
   */
  private async generateFallbackEmbedding(config: AgentConfig): Promise<number[]> {
    ...
    // Use the embedding dimension from options
    const dimension = this.options.embeddingDimension;
    const embedding: number[] = [];

    // Generate pseudo-random but deterministic values from hash
    for (let i = 0; i < dimension; i++) {
      // Use pairs of bytes from hash, repeating if necessary
      const byteIndex = (i * 2) % hash.length;
      const value = (hash[byteIndex] + hash[byteIndex + 1] * 256) / 65535; // Normalize to [0, 1]
      embedding.push((value - 0.5) * 2); // Center around 0 to [-1, 1]
    }
